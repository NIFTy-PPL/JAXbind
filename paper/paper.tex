...
system. It enables a user to retrieve arbitrary derivatives of
functions, batch computations, and just-in-time compile code for
additional performance. Its transformation system requires that all
components of the computation are written in JAX.
...
spherical harmonic transform from DUCC (Reinecke, 2024) to the
probabilistic programming package NIFTy (Edenhofer et al., 2024) as well
as the radio interferometry response from DUCC with the radio astronomy
package \texttt{resolve} (Arras et al., 2024). Furthermore, we intend to
connect the non-uniform FFT from DUCC with JAX for applications in
strong-lensing astrophysics. We envision many further applications
within and outside of astrophysics.
...
Enzyme-JAX (W. S. Moses \& Zinenko, 2024), which allows one to bind
arbitrary LLVM/MLIR, including C++, with automatically-generated (W.
Moses \& Churavy, 2020; W. S. Moses et al., 2021, 2022) or
manually-defined derivatives to JAX.

PyTorch (Ansel et al., 2024) and TensorFlow (Abadi et al., 2015) also
provide interfaces for custom extensions. PyTorch has an extensively
documented Python interface\footnote{\url{https://pytorch.org/docs/stable/notes/extending.html}}
...
R., Kaiser, L., Kudlur, M., \ldots{} Zheng, X. (2015).
\emph{{TensorFlow}: Large-scale machine learning on heterogeneous
systems}. \url{https://www.tensorflow.org/}
...
J., Gschwind, M., \ldots{} Chintala, S. (2024, April). {PyTorch} 2:
Faster machine learning through dynamic {P}ython bytecode transformation
and graph compilation. \emph{29th ACM International Conference on
...
Moses, W. S., \& Zinenko, O. (2024). \emph{{Enzyme-JAX}} (Version 0.0.6)
{[}Computer software{]}. \url{https://github.com/EnzymeAD/Enzyme-JAX}
