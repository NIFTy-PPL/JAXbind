...
\hypersetup{unicode=true,
            pdftitle={jax\_op: Use any function in JAX},
            pdfborder={0 0 0},
            breaklinks=true}
...
\title{\texttt{jax\_op}: Use any function in JAX}
...
backend. The aim of \texttt{jax\_op} is to drastically lower the burden
of connecting custom functions implemented in other programming
languages to JAX. Specifically, \texttt{jax\_op} provides an easy-to-use
...
In this paper, we present \texttt{jax\_op}, a package for bridging any
...
We believe \texttt{jax\_op} to be highly useful in scientific computing.
...
\texttt{jax\_op} package expects the Python function for \(f\) to take
...
\begin{Shaded}
\begin{Highlighting}[]
\ImportTok{import}\NormalTok{ jax\_linop }\CommentTok{\#FIXME}

\KeywordTok{def}\NormalTok{ f(out, args, kwargs\_dump):}
\NormalTok{    kwargs }\OperatorTok{=}\NormalTok{ jax\_linop.load\_kwargs(kwargs\_dump)}
...
provided, which \texttt{jax\_op} will register as the \texttt{jvp} of
...
primitive for our function \(f\) using the \texttt{jax\_op} package.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{f\_jax }\OperatorTok{=}\NormalTok{ jax\_linop.get\_nonlinear\_call(}
\NormalTok{    f, (f\_jvp, f\_vjp), f\_abstract, f\_abstract\_T}
\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\texttt{f\_jax} is a JAX primitive registered via the \texttt{jax\_op}
...
interface of \texttt{jax\_op} is, for simplicity, restricted to first
...
make use of these simplifications, \texttt{jax\_op} provides a special
...
Currently, \texttt{jax\_op} only has CPU but no GPU support. With some
